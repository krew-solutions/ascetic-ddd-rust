...
Fetches == {r \in OutboxEvents : r.event = "fetch"}
\* A fetch that took no slot names the group only: no slot of it had work.
NoFetches == {r \in OutboxEvents : r.event = "nofetch"}

Msgs == {r.msg : r \in Publishes}
Uris == {r.uri : r \in Publishes}
Groups == {r.d[1] : r \in Fetches} \cup {r.group : r \in NoFetches}
Slots == IF Fetches \cup NoFetches = {} THEN {0}
         ELSE 0..((CHOOSE r \in Fetches \cup NoFetches : TRUE).of - 1)
...
\* The slot a URI hashes to, read off the fetches: hashtext is not
...
Assign == [u \in Uris |-> IF FetchedBy(u) = {} THEN CHOOSE w \in Slots : TRUE
...
O == INSTANCE Outbox WITH Msgs <- Msgs, Uris <- Uris, Groups <- Groups, Slots <- Slots,
                          BatchSize <- BatchSize, MaxCrashes <- MaxCrashes,
                          VisibilityRule <- VisibilityRule, RehashAllowed <- RehashAllowed
...
RelevantIn(g, s, h) ==
  {m \in Msgs : /\ xid[m] # O!NoXid
                /\ xid[m] < h
                /\ assign[uriOf[m]] = s
                /\ O!Before(position[<<g, s>>], O!Key(m))}

\* A fetch speaks for its slot; a fetch that took no slot, for every slot.
Relevant(r) == IF r.event = "fetch" THEN RelevantIn(r.d[1], r.d[2], r.horizon)
               ELSE UNION {RelevantIn(r.group, s, r.horizon) : s \in Slots}

InBatch(r) == IF r.event = "fetch" THEN Range(r.msgs) ELSE {}

\* A full batch says nothing about what lies beyond its last message.
Beyond(r, m) == r.event = "fetch" /\ Len(r.msgs) = r.limit /\ O!Before(O!Key(r.msgs[Len(r.msgs)]), O!Key(m))
...
Logged ==
  ...
  /\ \/ /\ Step.event = "publish"
     ...
     \/ /\ Step.event = "nofetch"
        /\ Settled(Step)
        /\ \A s \in Slots : O!Eligible(<<Step.group, s>>, Step.horizon) = {}
